...
from extern.mvcontrol.unet_multi_branch_2 import UNet2DConditionModel
...
class MVControlPipelineOutput(BaseOutput):
    ...
    roughness: Union[List[PIL.Image.Image], np.ndarray, torch.Tensor]
    metallic: Union[List[PIL.Image.Image], np.ndarray, torch.Tensor]
...
class MVControlPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        ip_adapter = None,
        use_feature_full_token: bool = False,
    ): 
        super().__init__()
        ...
        self.use_feature_full_token = use_feature_full_token
    ...
    def _clip_encode_image(self, image, device, num_images_per_prompt, do_classifier_free_guidance, output_hidden_states=None, condition_mode='image', use_feature_full_token=False):
        dtype = next(self.image_encoder.parameters()).dtype
        image = self.image_processor(image, return_tensors="pt", do_rescale=True).pixel_values # [bsz, 3, h, w]->[bsz, 3, 224, 224]
        image = image.to(device=device, dtype=dtype)

        if use_feature_full_token:
            embeds = self.image_encoder(image, output_hidden_states=output_hidden_states).last_hidden_state
            if condition_mode == 'text':
                embeds = torch.zeros_like(embeds)
            elif condition_mode == 'image' or condition_mode == 'text-image':
                pass
            else:
                raise Exception(f'[INFO] Invalid condition mode :{condition_mode}.')
    ...
    def __call__(
        ...
        latents_r: Optional[torch.FloatTensor] = None,
        latents_m: Optional[torch.FloatTensor] = None,
        condition_mode: str = 'image', # 'image', 'text', 'text-image'
    ):
        r"""
        ...
        clip_image_embeds = self._clip_encode_image(image, device, num_images_per_prompt, do_classifier_free_guidance, 
                                output_hidden_states=True, condition_mode=condition_mode, use_feature_full_token=self.use_feature_full_token)
        ...
        # prepare latents for rmo
        if self.unet.is_multi_branch:
            latents_r = self.prepare_latents(
                batch_size * num_images_per_prompt,
                num_views,
                num_channels_latents,
                height,
            ...
            latents_m = self.prepare_latents(
                batch_size * num_images_per_prompt,
                num_views,
                num_channels_latents,
                height,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
                if self.unet.is_multi_branch:
                    latent_r_model_input = torch.cat([latents_r] * 2) if do_classifier_free_guidance else latents_r
                    latent_m_model_input = torch.cat([latents_m] * 2) if do_classifier_free_guidance else latents_m
                    concat_model_input = self.scheduler.scale_model_input(torch.cat([latent_model_input, latent_r_model_input, latent_m_model_input]), t)
                    latent_model_input, latent_r_model_input, latent_m_model_input = concat_model_input.chunk(3)
                ...
                # inference with rmo
                if self.unet.is_multi_branch:
                    noise_pred_r = self._forward( # noise_pred.shape=[2,4,4,32,32]
                        latent_model_input=latent_r_model_input,
                        controlnet_cond=hint, # hint.shape=[2,3,256,256], value=[0,1]
                        t=t, # tensor(981, device='cuda:0')
                        encoder_hidden_states=encoder_hidden_states, #encoder_hidden_states, # shape=[2,77,1024]
                        ip_adapter_embeds=clip_image_embeds,
                    ...
                    noise_pred_m = self._forward( # noise_pred.shape=[2,4,4,32,32]
                        latent_model_input=latent_m_model_input,
                        controlnet_cond=hint, # hint.shape=[2,3,256,256], value=[0,1]
                        t=t, # tensor(981, device='cuda:0')
                        encoder_hidden_states=encoder_hidden_states, #encoder_hidden_states, # shape=[2,77,1024]
                        ip_adapter_embeds=clip_image_embeds,
                ...
                if do_classifier_free_guidance:
                    ...
                    if self.unet.is_multi_branch:
                        noise_pred_r_uncond, noise_pred_r_text = noise_pred_r.chunk(2) # noise_pred_uncond, noise_pred_text shape=[1,4,4,32,32]
                        noise_pred_r = noise_pred_r_uncond + guidance_scale * (noise_pred_r_text - noise_pred_r_uncond)

                        noise_pred_m_uncond, noise_pred_m_text = noise_pred_m.chunk(2) # noise_pred_uncond, noise_pred_text shape=[1,4,4,32,32]
                        noise_pred_m = noise_pred_m_uncond + guidance_scale * (noise_pred_m_text - noise_pred_m_uncond)

                # if use_inner_batch:
                #     noise_pred = rearrange(noise_pred, '(b v) c h w -> b v c h w', v=inner_B)
                
                # compute the previous noisy sample x_t -> x_t-1
                #latents = self.scheduler.step(noise_pred, t, latents_rmo, **extra_step_kwargs).prev_sample
                if self.unet.is_multi_branch:
                    concat_latents = self.scheduler.step(
                        torch.cat([noise_pred, noise_pred_r, noise_pred_m]),
                        t,
                        torch.cat([latents, latents_r, latents_m]), 
                        **extra_step_kwargs
                    ).prev_sample
                    latents, latents_r, latents_m = concat_latents.chunk(3)
                else:
                ...
                if i == len(timesteps) - 1 or ((i + 1) > num_warmup_steps and (i + 1) % self.scheduler.order == 0):
                    ...
                    if callback is not None and i % callback_steps == 0:
                        ...
                        if self.unet.is_multi_branch:
                            callback(i, t, latents_r)
                            callback(i, t, latents_m)

        latents = rearrange(latents, 'b n c h w -> (b n) c h w')
        #latents_rmo = rearrange(latents_rmo, 'b n c h w -> (b n) c h w')
        ...
        if output_type == "latent":
            ...
            #rmo = self.decode_latents(latents_rmo)
            ...
            #rmo = self.decode_latents(rmo, post_procs=False)
            #rmo = rearrange(rmo, '(b n) c h w -> b n c h w', n=num_views)
        
        if output_type == "pil":
            image = self.numpy_to_pil(image)
            #rmo = self.numpy_to_pil(rmo)

            # 9. Run safety checker
            # image, has_nsfw_concept = self.run_safety_checker(image, device, prompt_embeds.dtype)
        if self.unet.is_multi_branch:
            latents_r = rearrange(latents_r, 'b n c h w -> (b n) c h w')
            latents_m = rearrange(latents_m, 'b n c h w -> (b n) c h w')
            if output_type == "latent":
                roughness = latents_r
                metallic = latents_m
                # has_nsfw_concept = None
                ...
                # 8. Post-processing
                roughness = self.decode_latents(latents_r)
                metallic = self.decode_latents(latents_m)

                # 9. Run safety checker
                # image, has_nsfw_concept = self.run_safety_checker(image, device, prompt_embeds.dtype)
            else:
                roughness = self.decode_latents(latents_r, post_procs=False)
                roughness = rearrange(roughness, '(b n) c h w -> b n c h w', n=num_views)

                metallic = self.decode_latents(latents_m, post_procs=False)
                metallic = rearrange(metallic, '(b n) c h w -> b n c h w', n=num_views)
            
            if output_type == "pil":
                roughness = self.numpy_to_pil(roughness)
                metallic = self.numpy_to_pil(metallic)
        ...
        if not return_dict:
            ...
            return image, rmo
        '''
        if not return_dict and self.unet.is_multi_branch:
            # return (image, has_nsfw_concept)
            return image, roughness, metallic
        elif not return_dict and not self.unet.is_multi_branch:
            return image

        if self.unet.is_multi_branch:
            return MVControlPipelineOutput(images=image, roughness=roughness, metallic=metallic)
        else:
            return MVControlPipelineOutput(images=image, roughness=None, metallic=None)
