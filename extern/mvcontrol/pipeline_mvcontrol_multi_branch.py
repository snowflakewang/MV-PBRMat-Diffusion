...
from extern.mvcontrol.unet_multi_branch import UNet2DConditionModel
...
class MVControlPipelineOutput(BaseOutput):
    ...
    rmo: Union[List[PIL.Image.Image], np.ndarray, torch.Tensor]
...
class MVControlPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        latents_rmo: Optional[torch.FloatTensor] = None,
        ...
        if self.unet.is_multi_branch:
            latents_rmo = self.prepare_latents(
                batch_size * num_images_per_prompt,
                num_views,
                num_channels_latents,
                height,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
                if self.unet.is_multi_branch:
                    latent_rmo_model_input = torch.cat([latents_rmo] * 2) if do_classifier_free_guidance else latents_rmo
                    concat_model_input = self.scheduler.scale_model_input(torch.cat([latent_model_input, latent_rmo_model_input]), t)
                    latent_model_input, latent_rmo_model_input = concat_model_input.chunk(2)
                ...
                # inference with rmo
                if self.unet.is_multi_branch:
                    noise_pred_rmo = self._forward( # noise_pred.shape=[2,4,4,32,32]
                        latent_model_input=latent_rmo_model_input,
                        controlnet_cond=hint, # hint.shape=[2,3,256,256], value=[0,1]
                        t=t, # tensor(981, device='cuda:0')
                        encoder_hidden_states=encoder_hidden_states, #encoder_hidden_states, # shape=[2,77,1024]
                        ip_adapter_embeds=clip_image_embeds,
                ...
                if do_classifier_free_guidance:
                    ...
                    if self.unet.is_multi_branch:
                        noise_pred_rmo_uncond, noise_pred_rmo_text = noise_pred_rmo.chunk(2) # noise_pred_uncond, noise_pred_text shape=[1,4,4,32,32]
                        noise_pred_rmo = noise_pred_rmo_uncond + guidance_scale * (noise_pred_rmo_text - noise_pred_rmo_uncond)
                ...
                if self.unet.is_multi_branch:
                    concat_latents = self.scheduler.step(
                        torch.cat([noise_pred, noise_pred_rmo]),
                        t,
                        torch.cat([latents, latents_rmo]), 
                        **extra_step_kwargs
                    ).prev_sample
                    latents, latents_rmo = concat_latents.chunk(2)
                else:
        ...
        if self.unet.is_multi_branch:
            return MVControlPipelineOutput(images=image, rmo=rmo)
        else:
            return MVControlPipelineOutput(images=image, rmo=None)
